...
contract Vault is OwnableUpgradeSafe {
        ...
        public
    {
        ...
        uint256 amount = stakeTokenIsLBD() ? tokensToShares(tokens) : tokens;
        ...
        public
    {
        ...
        bool ok = LBD.transfer(msg.sender, tokens);
        ...
        public
    {
        ...
        uint256 amount = stakeTokenIsLBD() ? sharesToTokens(staked[msg.sender]) : staked[msg.sender];
        ...
        )
    {
        ...
        _maxStaked = stakeTokenIsLBD() ? sharesToTokens(maxStaked) : maxStaked;
        _totalTokensStaked = stakeTokenIsLBD() ? sharesToTokens(totalStaked) : totalStaked;
        _tokensStakedByUser = stakeTokenIsLBD() ? sharesToTokens(staked[user]) : staked[user];
    ...
    function addRewards(uint256 tokens)public onlyOwner
    {
        totalRewardShares = totalRewardShares.add(tokensToShares(tokens));
        bool ok = LBD.transferFrom(msg.sender, address(this), tokens);
        require(ok, "transfer");
        emit AddRewards(tokens, sharesToTokens(totalRewardShares));
    }

    function removeRewards(uint256 tokens)public onlyOwner
    {
    ...
    function stakeTokenIsLBD()
        private
        view
        returns (bool)
    {
        return address(stakeToken) == address(LBD);
    }
    ...
    function sharesToTokens(uint256 shares)
        ...
        returns (uint256)
    {
        return shares.mul(LBD.totalSupply()).div(LBD.totalShares());
    }
     ...
     function tokensToShares(uint256 tokens)
        ...
        returns (uint256)
    {
        return tokens.mul(LBD.totalShares().div(LBD.totalSupply()));
    }
}
